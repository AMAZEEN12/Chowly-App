...
export default function StaffLoginPage() {
  ...
  return <main className="auth-page container"><div className="auth-card staff-card">
    ...
    <form onSubmit={submit} className="form-stack"><label>Email<input type="email" value={email} onChange={e => setEmail(e.target.value)}/></label><label>Password<PasswordInput value={password} onChange={e => setPassword(e.target.value)} autoComplete="current-password" /></label>{error && <p className="error">{error}</p>}<Link className="text-link" to="/staff/forgot-password">Forgot password?</Link><button className="btn big full">Login as staff</button></form>
